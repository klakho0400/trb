...
def get_historic_klines_stoch(symbol, start, end, interval):
    klines = get_historical_klines(symbol, interval, start, end)
    ochl = []
    for kline in klines:
        time1 = int(kline[0])
    ...
    data = pd.DataFrame(ochl, columns=['time', 'Open', 'Close', 'High', 'Low', 'Volume']).reset_index().to_dict(
        orient='list')
...
def buy(hist,p, source):
    ...
    for i in range(4):
        print()
        if p[i]<p[i+1] and hist[i]<hist[i+1]:
            print('bought btcusdt at' + str(source[i]))
            if(source[i] not in b):
                b.append(source[i])
                prev_b.append(source[i])
        else:
            if(len(b)!=0):
                if p[i] >= p[i + 1] or hist[i] >= hist[i + 1]:
                    x = b.pop()
                    print('sold at' + str(source[i]))
                    print('profit' + str(source[i]-x))
                    prof.append(source[i]-x)
                    s.append(source[i])
    print(prev_b)
    print(s)
    print(sum(prof))
...
if __name__ == '__main__':
    while True:
        ...
        macdo = pd.DataFrame(macd)
        histo = pd.DataFrame(hist)
        signalo = pd.DataFrame(signal)
        ...
        buy(hist,p, source)
        ax2.plot(range(0, len(p)), p * 100, linewidth=2)
        ax2.legend(['Stochastic'])
